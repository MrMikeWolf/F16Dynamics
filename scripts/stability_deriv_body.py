...
def stability_deriv_body(eqm_fun, X0, controls, params, dX=None):
    ...
    # u derivatives
    X_up = X0
    X_up[1] = X_up[1] + df[1]/2
    X_down = X0
    X_down[1] = X_down[1] - dX[1]/2
    [XD_up, out_up] = eqm_fun(0, X_up, controls, params)
    [XD_down, out_down] = eqm_fun(0, X_down, controls, params)
    long.CXu = (out_up.aero_forces[1] - out_down.aero_forces[1])/df[1]
    long.CZu = (out_up.aero_forces[3] - out_down.aero_forces[3])/df[1]
    long.CMu = (out_up.aero_moments[2] - out_down.aero_moments[2])/df[1]

    # w derivatives
    X_up = X0
    X_up[3] = X_up[3] + df[3]/2
    X_down = X0
    X_down[3] = X_down[3] - df[3]/2
    [XD_up, out_up] = eqm_fun(0, X_up, controls, params)
    [XD_down, out_down] = eqm_fun(0, X_down, controls, params)
    long.CXw = (out_up.aero_forces[1] - out_down.aero_forces[1])/df[3]
    long.CZw = (out_up.aero_forces[3] - out_down.aero_forces[3])/df[3]
    long.CMw = (out_up.aero_moments[2] - out_down.aero_moments[2])/df[3]
    ...
    long.Thrust_v = (out_up.thrust_pound - out_down.thrust_pound)/df[1]
    
    # Pitch derivatives
    X_up = X0
    X_up[8] = X_up[8] + df[8]/2
    X_down = X0
    X_down[8] = X_down[8] - df[8]/2
    [XD_up, out_up] = eqm_fun(0, X_up, controls, params)
    [XD_down, out_down] = eqm_fun(0, X_down, controls, params)
    long.CXq = (out_up.aero_forces[1] - out_down.aero_forces[1])/df[8]
    long.CZq = (out_up.aero_forces[3] - out_down.aero_forces[3])/df[8]
    long.CMq = (out_up.aero_moments[2] - out_down.aero_moments[2])/df[8]
